...
private endText(): void {
  ...
  const titlee= new BitmapText('End Game',{fontName:'Desyrel', fontSize: 120,
  align: 'center',});
  titlee.anchor.set(0.5,0.5);
  titlee.x =this.app.view.width/2;
  titlee.y = titlee.height;
  const stop = new BitmapText('End Game', {
    fontName: 'Desyrel',
    fontSize: 50,
    align: 'center',
  });
